...
describe('GET /peer/transactions', function () {
	...
	it('using vendorField should be ok', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 1, "this is a test vendorfield", node.gAccount.password);
...
describe('POST /peer/transactions', function () {
	...
	it('using valid headers should be ok', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 1, null, node.gAccount.password);
	...
	it('using already processed transaction should be not ok (preventing spam)', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 2, null, node.gAccount.password);
	...
	it('using already confirmed transaction should be not ok', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 3, null, node.gAccount.password);
	...
	it('when sender has no funds should fail', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 1, null, 'randomstring');
			...
			node.onNewBlock(function () {
				...
				node.async.doUntil(function (next) {
					postTransaction(transaction2, function (err, res) {
						node.expect(res.body).to.have.property('success').to.be.not.ok;
						node.expect(res.body).to.have.property('message').to.match(/Account does not have enough ARK: [a-zA-Z0-9]+A balance: 1e-8/);
						count++;
						return next();
					});
				}, function () {
	...
	it('using very large amount and genesis block id should fail', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 100000000000000000, null, node.gAccount.password);
	...
	it('using overflown amount should fail', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 184819291270000000012910218291201281920128129, null,
	...
	it('using float amount should fail', function (done) {
		var transaction = node.ark.transaction.createTransaction('C2uLD1g2wddJVasKZkHrRu53YztWRYQzyeeSduzGWWteh8y8zA', 1.3, null, node.gAccount.password);
