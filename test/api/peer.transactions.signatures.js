...
describe('POST /peer/transactions', function () {
	...
	describe('using second signature', function () {
		...
		it('when account does not have one should fail', function (done) {
			var transaction = node.ark.transaction.createTransaction('1L', 1, node.gAccount.password, account.secondPassword);
		...
		it('using blank second passphrase should fail', function (done) {
			var transaction = node.ark.transaction.createTransaction('1L', 1, account.password, '');
		...
		it('using fake second passphrase should fail', function (done) {
			var transaction = node.ark.transaction.createTransaction('1L', 1, account.password, account2.secondPassword);
		...
		it('using valid second passphrase should be ok', function (done) {
			var transaction = node.ark.transaction.createTransaction('1L', 1, account.password, account.secondPassword);
